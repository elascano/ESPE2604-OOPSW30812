# Author: Didier Elbay
# Farm Management System - Main View
...
CONNECTION_STRING = "mongodb+srv://YOUR_USER:YOUR_PASSWORD@YOUR_CLUSTER.mongodb.net/?appName=FarmZoo"
...
class FarmSystemApp:
    def __init__(self, root):
        ...
        # Start with no menu until the user logs in
    ...
    def show_login_frame(self):
        ...
        tk.Button(
            self.main_container, text="Login", command=login_validate,
    ...
    def show_home(self):
        ...
        tk.Label(
            self.main_container,
            text="Welcome To Farm Management System",
            font=("Arial", 14, "bold")
        ).pack(pady=150)
    ...
    def show_chicken_frame(self):
        ...
        tk.Checkbutton(self.main_container, text="Is Molting Plumes?", variable=chk_molting_var).pack(pady=5)
    ...
    def show_cow_frame(self):
        ...
        def save():
            try:
                ...
                doc = {
                    "id": id_val, "milk": milk, "weight": weight,
                    "efficiency": round(efficiency, 2),
                    "meatYield": round(expected_meat, 2),
                    "date": datetime.now()
                }
                db["cows"].insert_one(doc)

                lbl_result.config(text=f"Efficiency: {efficiency:.1f}% | Meat: {expected_meat:.2f} kg")
                messagebox.showinfo("Success", "Cow saved successfully!")
            except ValueError:
    ...
    def show_pig_frame(self):
        ...
        def save():
            try:
                ...
                expected_meat = round((weight * 0.15) + (weight * 0.10), 2)

                doc = {
                    "id": id_val, "breed": breed, "weight": weight,
                    "expectedMeat": expected_meat, "date": datetime.now()
                }
                db["pigs"].insert_one(doc)

                lbl_result.config(text=f"Expected Meat: {expected_meat} kg")
                messagebox.showinfo("Success", "Pig record saved successfully!")
            except ValueError:
                messagebox.showerror("Error", "Please verify your numeric values.")
    ...
    def show_sheep_frame(self):
        ...
        def save():
            try:
                ...
                lbl_result.config(text=f"Status: {status}")
                messagebox.showinfo("Success", "Sheep record saved successfully!")
            except ValueError:
