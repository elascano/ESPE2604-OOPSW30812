...
`;
        ...
        guiProcess.stdout.on('data', async (data) => {
            try {
                const event = JSON.parse(data.trim());
                if (event.action === "SAVE_PERSON") {
                    const p = new Person(event.data.id, event.data.name, event.data.email);
                    await this.controller.saveOrUpdatePerson(p);
                } else if (event.action === "SAVE_PRODUCT") {
                    const prod = new Product(event.data.id, event.data.name, parseFloat(event.data.price || 0));
                    await this.controller.saveOrUpdateProduct(prod);
                } else if (event.action === "SAVE_PAYMENT") {
                    const pay = new Payment(event.data.id, event.data.method, parseFloat(event.data.amount || 0));
                    await this.controller.saveOrUpdatePayment(pay);
                } else if (event.action === "LOAD_PERSON") {
                    const res = await this.controller.findPerson(event.data.id);
                    console.log("\n--- [Node.js MVC] Person Loaded ---", res);
                } else if (event.action === "LOAD_PRODUCT") {
                    const res = await this.controller.findProduct(event.data.id);
                    console.log("\n--- [Node.js MVC] Product Loaded ---", res);
                } else if (event.action === "LOAD_PAYMENT") {
                    const res = await this.controller.findPayment(event.data.id);
